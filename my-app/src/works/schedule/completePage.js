...
class Page extends Component{
    ...
    createPage(pageNum,current){
        ...
        pages.push(
            <Link onClick={this.prevClick.bind(this)}
                  to={{
                    pathname: `/completeSchedule/${current-1}`,
                    state: { id: current-1 }
                }}
                  key={0}>
        ...
        if (pageNum <= 5) {
            ...
            pages.push(<Link  key={pageNum}
                              to={{
                        state: { id: pageNum },
                        pathname: `/completeSchedule/${pageNum}`
                    }}
    ...
    pageClick(current) {
        let { getOtherData,data} = this.props;
